...
"""
nb2tex.py is a Python script used to convert a directory and subdirectories of Jupyter notebooks
and convert these notebooks into one big .tex file. After conversion from multiple notebooks to one text file,
the resulting .tex document can be complied to .pdf by a LaTeX engine such as TeXworks.
...
The resulting .tex file is saved to the pdf directory.
...
def export(combined_nb: NotebookNode, output_file: Path, pdf=False, template_file=None):
    resources = {}
